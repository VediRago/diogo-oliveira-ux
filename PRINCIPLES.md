...
Every case study owns its own evidence. Each Vault belongs to one case study, and every artifact inside it returns to that case study — no cross-linking between Vaults. If two case studies need the same artifact, each gets its own copy. Ownership stays local. Navigation stays predictable.
